...
if __name__ == "__main__":
    ...
    if dynamic_simulation:
        ...
        if omega_temperature != 0:
            m.fs.objective = pyo.Objective(expr=m.fs.total_power_obj + sum(
            ...
            m.fs.objective = pyo.Objective(expr=m.fs.total_power_obj, sense=pyo.minimize)
        ...
        if keep_controllers:
            ...
            m.fs.objective = pyo.Objective(expr=m.fs.total_power_obj 
                                        , sense=pyo.minimize)
        ...
        if os.path.exists(file):
            ...
            obj_val_base = pyo.value(m.fs.total_power_obj)
...
for weight in weights:
    ...
    average_power = sum(m.fs.total_electric_power[t]() * (t - m.fs.time.prev(t)) for t in list(m.fs.time)[1:]) / m.fs.time.last()
